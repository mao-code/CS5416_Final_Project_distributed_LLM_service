...
TOTAL_NODES = int(os.environ.get('TOTAL_NODES', 2))
...
class MonolithicPipeline:
    ...
    def process_batch(self, requests: List[PipelineRequest]) -> List[PipelineResponse]:
        ...
        print("Document fetch time:", time.time() - start)
        ...
        print("Document rerank time:", time.time() - start)
        ...
        print("Safety filter time:", time.time() - start)
...
def embed_query():
    ...
    try:
        print("oh")
        data = request.data # this is the list of queries
        print("OKay? ", data)

        return jsonify(pipeline.process_request(data)), 200
...
def main():
    ...
    if NODE_NUMBER == 0:
        ...
        pipeline = EmbeddingNode()
        print("Embedding node created")
